...
def get_ground_truth_names(g_folder):
    # assert (year == 17)
    full_gt_names = []
    for patient_name in os.listdir(g_folder):
        patient_dir = os.path.join(g_folder, patient_name)
        full_gt_names.append(patient_dir)
    return full_gt_names


def get_segmentation_names(seg_folder):
    full_seg_names = []
    for patient_name in os.listdir(seg_folder):
...
def brats_metics(s_folder, g_folder):
    # year = 17
    ...
    gt_names = sorted(get_ground_truth_names(g_folder))
    seg_names = sorted(get_segmentation_names(s_folder))
    for index in range(len(gt_names)):
        a = gt_names[index]
        b = seg_names[index]
        a = a.split('/')[-1]
        b = b.split('/')[-1]
        ...
        if a != b:
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    os.makedirs(os.path.join(s_folder, 'txt'), exist_ok=True)
    ...
    for type_idx in range(3):
        ...
        np.savetxt(s_folder + '/dice_{0:}.txt'.format(test_type+str(dice_mean)+"+-"+str(dice_std)), dice)
    ...
    if "test1" in s_folder:
        ...
        for type_idx in range(3):
            ...
            test_type = test_types[type_idx]
            np.savetxt(s_folder + '/haus_{0:}.txt'.format(test_type+str(haus_mean)+"+-"+str(haus_std)), haus)
