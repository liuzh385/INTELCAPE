...
class MergeOperator(nn.Module):
    def __init__(self):
        super().__init__()
        self.alphas = torch.tensor(0.0)
        self.alpha_squashed = torch.sigmoid(self.alphas)

    def __call__(self, base_encoding, side_encoding):
        # Note: returns alpha_squashed * base_encoding + (1 - alpha_squashed) * side_encoding when not dense
        merged_encoding = self.alpha_squashed * base_encoding + (1 - self.alpha_squashed) * side_encoding
        # merged_encoding = self.alpha_squashed * base_encoding + side_encoding
        # print(type(merged_encoding), type(base_encoding))
        # print(merged_encoding.device, base_encoding.device)
