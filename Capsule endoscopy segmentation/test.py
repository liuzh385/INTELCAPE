...
def main(args, cfg):
    ...
    if args.mode == "train":
        ...
        bingo = 0
        total = 0
        avi_path = "/mnt/minio/node77/liuzheng/pk/avi"
        csv_path = "/mnt/minio/node77/liuzheng/pk/csv/csv.csv"
        ...
        for i in range(3):
            ...
            avi_name = os.path.join(avi_path, sample_name, sample_name + '.avi')
            tmp = cv2.VideoCapture(avi_name)
            frame_num = int(tmp.get(7))
            with state:
                start_time = time.time()
                for j in range(30):
                    index = random.randint(0, stomach_frame)
                    target = 0
                    ...
                    total += 1
                for j in range(30):
                    index = random.randint(stomach_frame + 1, s_intestine_frame)
                    target = 1
                    tmp.set(cv2.CAP_PROP_POS_FRAMES, index)
                    _, frame = tmp.read()
                    frame[305:, :131, :] = [0, 0, 0]
                    ...
                    total += 1
                for j in range(30):
                    index = random.randint(s_intestine_frame + 1, frame_num)
                    target = 2
                    tmp.set(cv2.CAP_PROP_POS_FRAMES, index)
                    _, frame = tmp.read()
                    frame[305:, :131, :] = [0, 0, 0]
