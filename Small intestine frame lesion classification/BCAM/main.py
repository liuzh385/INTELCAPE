...
class Trainer(object):
    ...
    def _set_optimizer(self):
        ...
        for name, parameter in self.model.named_parameters():
            ...
            if string_contains_any(
                ...
                elif self.args.architecture in ('resnet50','resnet34'):
                    if 'weight' in name:
    ...
    def evaluate(self, epoch, split):
        ...
        if self.args.dataset_name in ('CUB', 'ILSVRC', 'Crohn15to23'):
...
def main():
    ...
    for epoch in range(trainer.args.epochs):
        ...
        if (epoch + 1) % trainer.args.eval_frequency == 0:
            trainer.evaluate(epoch + 1, split='test')
            trainer.print_performances()
            trainer.report(epoch + 1, split='test')
